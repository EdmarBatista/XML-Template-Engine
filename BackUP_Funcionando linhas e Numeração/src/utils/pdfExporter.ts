...
// Inicializa fontes virtuais do pdfmake
try {
  const vfsObj = (pdfFonts as any)?.pdfMake?.vfs || (pdfFonts as any)?.vfs || pdfFonts;
  if (vfsObj) {
    (pdfMake as any).vfs = vfsObj;
  }
} catch (e) {
  console.warn('Aviso ao registrar vfs_fonts do pdfmake em pdfExporter:', e);
}
