...
import { aplicarMascaraCampo, normalizarValorCampo } from '../utils/documentUtils';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/75 backdrop-blur-xs flex items-center justify-center p-3 sm:p-5">
      <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-5xl h-[88vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        {/* Tabs Bar */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-2 bg-slate-100 border-b border-slate-200 shrink-0 flex-wrap gap-2">
          ...
          {(tab === 'json-dados' || tab === 'xml-edit' || tab === 'json-modelo') && (
            <div className="flex items-center gap-1.5 ml-auto">
              <button
                ...
                className="flex items-center gap-1 text-xs px-2.5 py-1 bg-white border border-slate-300 rounded text-slate-700 hover:bg-slate-50 transition"
                title={
                  tab === 'json-dados'
                    ? 'Copiar dados JSON'
                    : tab === 'xml-edit'
          ...
          <div className="px-6 py-2.5 bg-white border-b border-slate-200 flex items-center justify-between gap-3 flex-wrap shrink-0">
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                ...
                className="w-full text-xs pl-8 pr-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-md focus:bg-white focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
            ...
            <div className="flex items-center gap-1 text-xs">
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded transition ${
                  filtroStatus === 'todos' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                }`}
              >
                Todos ({chaves.length})
              </button>
              <button
                ...
                className={`px-2.5 py-1 rounded transition ${
                  filtroStatus === 'preenchidos' ? 'bg-emerald-700 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                }`}
              >
                Preenchidos
              </button>
              <button
                ...
                className={`px-2.5 py-1 rounded transition ${
                  filtroStatus === 'vazios' ? 'bg-amber-700 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                }`}
              >
                Pendentes
              </button>
        ...
        {/* Modal Body Content */}
        <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 bg-slate-50 flex flex-col">
          {tab === 'vars-edit' && (
              ...
              {chavesFiltradas.length === 0 ? (
                ...
                chavesFiltradas.map(id => {
                  ...
                  return (
                    <div
                      key={id}
                      className="p-3 bg-white rounded-lg border border-slate-200 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-3"
                    >
                      <div className="md:w-1/3">
                        <div className="flex items-center gap-1.5">
                          <code className="text-xs font-bold text-blue-700 font-mono bg-blue-50 px-1.5 py-0.5 rounded">
                            {`{{${id}}}`}
                          </code>
                        ...
                        <p className="text-xs font-semibold text-slate-800 mt-1">{campo?.label}</p>
                        {campo?.descricao && (
          ...
          {tab === 'vars-readonly' && (
            <div className="flex-1 min-h-[380px] h-full bg-white rounded-lg border border-slate-200 shadow-2xs flex flex-col overflow-hidden">
              <div className="flex-1 min-h-0 overflow-auto">
                <table className="w-full text-xs text-left border-collapse">
                  <thead className="sticky top-0 z-20 bg-slate-100">
                    <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200">
                      <th className="sticky top-0 z-20 p-3 bg-slate-100 font-bold text-slate-700 border-b border-slate-200 shadow-xs whitespace-nowrap">
                        Identificador
                      </th>
                      <th className="sticky top-0 z-20 p-3 bg-slate-100 font-bold text-slate-700 border-b border-slate-200 shadow-xs whitespace-nowrap">
                        Rótulo (Label)
                      ...
                      chavesFiltradas.map(id => {
                        ...
                        return (
                          <tr key={id} className="hover:bg-slate-50/70 transition-colors">
                            <td className="p-3 font-mono font-bold text-blue-700 whitespace-nowrap">{`{{${id}}}`}</td>
                            <td className="p-3 text-slate-800 font-medium">{campo?.label || '-'}</td>
                            <td className="p-3 text-slate-500 font-mono text-[11px] uppercase">{campo?.tipo || 'texto'}</td>
                            <td className="p-3">
                              {temValor ? (
                                <span className="text-slate-900 font-medium bg-emerald-50 text-emerald-800 px-2 py-0.5 rounded border border-emerald-200 inline-block max-w-md truncate">
                                  {String(valor)}
                                </span>
                              ) : (
          ...
          {/* JSON DADOS TAB: Visualização e Edição com Realce de Sintaxe Colorido */}
          {tab === 'json-dados' && (
