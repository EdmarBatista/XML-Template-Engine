...
export const DocumentInlineVariable: React.FC<DocumentInlineVariableProps> = ({
  ...
  if (editando) {
    if (campo?.tipo === 'radio') {
      ...
      return (
        <span
          ...
          className="inline-flex items-center gap-1.5 flex-wrap bg-white border-2 border-blue-500 rounded-md px-2 py-0.5 shadow-md z-20 relative my-0.5 align-middle"
        >
          {opcoes.map((opt, i) => {
    ...
    if (campo?.tipo === 'checkbox') {
      ...
      return (
        <span
          ...
          onClick={e => e.stopPropagation()}
          onMouseDown={e => e.stopPropagation()}
          className="inline-flex items-center gap-1.5 bg-white border-2 border-blue-500 rounded-md px-2 py-0.5 shadow-md z-20 relative my-0.5 align-middle"
        >
      ...
      return (
        ...
        >
          <input
            ...
            className="bg-white border-2 border-blue-500 rounded px-2 py-1 text-xs text-slate-900 font-sans outline-none shadow-md font-medium cursor-pointer"
          />
        </span>
      );
    }
      ...
      return (
        ...
        >
          <select
            ...
            className="bg-white border-2 border-blue-500 rounded px-2.5 py-1 text-xs text-slate-900 font-sans outline-none shadow-md font-medium"
          >
      ...
      return (
        ...
        >
          <textarea
            ...
            className="w-full bg-white border-2 border-blue-500 rounded p-2 text-xs text-slate-900 font-sans outline-none shadow-md"
          />
        </span>
      );
    }
    ...
    return (
      ...
      >
        <input
          type={campo?.tipo === 'number' && !isMasked ? 'number' : 'text'}
          ...
          className="bg-white border-2 border-blue-500 rounded px-2.5 py-1 text-xs text-slate-900 font-sans outline-none shadow-md font-medium min-w-[200px] max-w-full"
        />
      </span>
    );
  }
      ...
      className={`inline cursor-pointer px-1 py-0.5 rounded border transition-colors duration-500 ease-in-out select-text ${
        hasValue
          ? isHighlighted
            ...
            : variaveisVermelhasWord
            ? 'text-red-600 bg-red-50 border-transparent hover:bg-red-100 hover:border-red-400'
            : 'text-slate-900 bg-blue-50/80 border-transparent hover:bg-blue-100 hover:border-blue-400'
          : isHighlighted
          ? 'bg-emerald-200 text-emerald-950 font-mono text-xs border-emerald-500'
          : 'text-amber-700 bg-amber-100/90 font-mono text-xs border-amber-300 hover:bg-amber-200'
      }`}
    >
